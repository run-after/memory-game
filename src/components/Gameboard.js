...
function Gameboard(props) {
  ...
  const handleSelection = (e) => {
    ...
    if (cards[id].clicked) {
      alert('I told you not to select the same card twice...');
      props.gameOver();
      resetClickedStatus();
    } else {
      setCards((prevState) => ({ 
  ...
  return (
    <div className='gameboard'>
      {randomOrderedState.map((key) => {
        
        return (
          <div id={key} key={key} className='card' onClick={handleSelection}>
